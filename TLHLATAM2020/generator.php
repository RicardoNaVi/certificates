<?php
	...
	$sql = "SELECT id,Email,UPPER(Nombre) as nameP,Downloads FROM $Table WHERE email='$email' AND id= '$pedido'";
	$resultado = $conn->query($sql);
	$code = "0";
	if ($resultado -> num_rows > 0) {
		try {
			$row = $resultado -> fetch_assoc();
			$sql = "UPDATE $Table SET Downloads=Downloads+1 WHERE email='$email' AND id= '$pedido'";
			$conn -> query($sql);
			...
			imagettftext($im, 80, 0, $center, 1120, $color, $fuente, $row["nameP"]);
			imagepng($im);
			$code = "0";
		} catch (Exception $e) {
			//throw $th;
